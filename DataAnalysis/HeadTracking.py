...
def update_surbrillance(pitch, heading):
    ...
    elif index == aoic.RIGHT:
        gZone_right.set_alpha(0.7)
    elif index == aoic.FRONT:
...
if __name__ == "__main__":
    ...
    gZone_left = ZoneGraphics("left", zones[0], (1, 0, 0))
    gZone_right = ZoneGraphics("right", zones[1], (0, 1, 0))
    gZone_front = ZoneGraphics("front", zones[2], (1, 1, 0))
    gZone_zi1 = ZoneGraphics("ZI1", zones[3], (0, 0, 1))
    gZone_zi2 = ZoneGraphics("ZI2", zones[4], (0, 1, 1))

    # Launch animation
    ani = FuncAnimation(fig, update_animation, init_func=init_animation, frames=[i for i in range(5000, len(data))], blit=True, interval = 5)
    plt.show()